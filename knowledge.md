# Knowledge Base
Last updated: 2026-08-09T13:47:56.025172
...
Category: gotcha
Tags: dev-reader, thousands, separator, recombination, gate, las-port, regression
Changed: 2026-08-09T13:47:38.132877

Thousands-recombination gates need HYBRID full-row + per-run fallback: a pure full-row exact-fit gate ('post_count == expected' over the whole row) silently rejects runs that individually satisfy expected when the full-row count does not match — regressing previously-correct shapes (headerless first row with 2+ runs; surplus row mixing an unambiguous run with a genuine 3-digit pair; BOTH the DEV reader and the LAS port). M-33 (MEDIUM): per-run post_count assumed ONLY the current run merges → two single-group values '1,234.5,2,345.6' in a 2-col row never recombine (full-row recombination would satisfy expected). The full-row fix regressed F-02 (HIGH, s12): full-row post-merge count 2 != expected 3 rejected ALL runs including ones the per-run snapshot would merge; OLD per-run gate had merged them correctly. H-03 (HIGH): headerless first-row derivation _expected_cols = len(values)-1 defeats the DEV-02 exact-fit gate for genuine multi-column shapes (headerless '100,250.5' → one column [100250.5]); the -1 derivation must not apply when the first row is a genuine multi-column shape. Fix: keep the full-row pre-scan as PRIMARY (fixes M-33), fall back to per-run snapshot evaluation (post_count = len(out) + 1 + (n - j)) when full-row != expected; every gate widening/narrowing must be mirrored across ALL delimiter paths AND the LAS port (_data_section_reader.py grammar twin). Checklist: test shapes (a) headerless 2+ runs, (b) mixed run + 3-digit pair surplus, (c) M-33 primary two-single-group, (d) short-row unambiguous; preserve M-76 run_len>=3 exemption and E-24 equal-count widening; the LAS port needs the identical change (M-32 grammar drift was shared).
